...
7. **Simplicidad y código claro**: los nombres deben expresar intención y cada función, clase o módulo debe tener una responsabilidad clara. No se conserva código muerto, comentado o duplicado; los comentarios explican el porqué, no repiten el código. Las nuevas capas, value objects, eventos y abstracciones solo se incorporan ante una necesidad actual verificable. `app.ts` compone la aplicación y `index.ts` se limita a iniciar el proceso.
